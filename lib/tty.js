...
function Session(server, socket) {
  ...
  var poll = false;
  if (conf.usePolling) poll = conf.usePolling;
  var self = this;
  var watcher = chokidar.watch(static, {persistent: true, ignoreInitial: true, usePolling: poll });
  self.log('Watching directory %s', static);
  watcher
        .on('add', function(filepath) {
            var filename = path.basename(filepath)
            if (filepath.match(/html$/)) {
                self.log('Watch Event [add] on file: %s -> rendering template into DOM.', filename);
                self.socket.emit('data', -1, filename);}})
        .on('change', function(filepath) {
            var filename = path.basename(filepath)
            if (filepath.match(/html$/)) {
                self.log('Watch Event [change] on file: %s -> rendering template into DOM.', filename);
                self.socket.emit('data', -2, filename);}})
        .on('error', function(error) {self.log('Error happened', error);})
        // .on('all', function(event, filepath) {
        //     self.log('Event %s occured on file %s', event, filepath);})
}
