...
function Session(server, socket) {
  ...
  fs.watch(static, function(event, filename) {
      var path = filename;
      if (event == 'rename' && path.match(/html$/)) {
          self.log('Watch Event [%s] on file: %s -> rendering template into DOM.', event, filename);
          self.socket.emit('data', -1, path);
      } else if (event == 'change' && path.match(/html$/)) {
          self.log('Watch Event [%s] on file: %s -> re-rendering template into DOM.', event, filename);
          self.socket.emit('data', -2, path);
      }
  });
}
